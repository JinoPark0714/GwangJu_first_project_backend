...
async def get_webtoon_info(new_title, new_description):
    # 함수 전처리
    okt = Okt()
    ...
    tfidf_desc_vect = load('./src/py_model/tfidf_desc_vect.joblib')
    with open('./src/py_model/clean_desc_list.pickle', 'rb') as file:
        clean_desc_list = pickle.load(file)
    with open('./src/py_model/clean_title_list.pickle', 'rb') as file:
        clean_title_list = pickle.load(file)

    # Load the tokenizer
    with open('./src/py_model/tokenizer.pickle', 'rb') as file:
        tokenizer = pickle.load(file)

    # Load the pre-trained model
    model = load_model('./src/py_model/best-LSTM-model.h5')
    ...
    def predict_rating(title, description):
        ...
        predicted_rating = model.predict([padded_title, padded_description], verbose=0)
    ...
    # 추천 줄거리 제공
    openai.api_key = "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"
    response = openai.ChatCompletion.create(
         model="gpt-3.5-turbo",
         messages=[{
             "role": "user", 
             "content": f"I'm planning to create a new webtoon with the title {new_title} and the description {new_description}. Can you write a plot for this story? I don't need a response, just show the plot. Answer in Korean"}]
    )
...
async def main():
    ...
    try:
        ...
        await get_webtoon_info(new_title, new_description)
